  ...
  return (
    <div className="p-8 bg-gray-50 min-h-screen">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        {basvurular.length === 0 ? (
          <p className="text-center text-gray-600 col-span-full">Bu ilana henüz başvuru yapılmamış.</p>
        ) : (
          basvurular.map((aday) => (
            <div key={aday.basvuru_id} className="border p-6 bg-white rounded-lg shadow">
              <h2 className="text-xl font-semibold text-green-700">{aday.aday_adi}</h2>
              <p className="text-sm text-gray-600">Başvuru ID: {aday.basvuru_id}</p>
              <button
                className="mt-4 w-full bg-blue-600 hover:bg-blue-800 text-white py-2 rounded"
                onClick={() => navigate("/juri-detay", { state: { aday } })}
              >
                İncele
              </button>
            </div>
