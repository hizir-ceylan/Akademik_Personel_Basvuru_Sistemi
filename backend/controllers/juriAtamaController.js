...
exports.getAtananJuriler = async (req, res) => {
  ...
  try {
    const result = await pool.query(`
      SELECT k.id, 
             COALESCE(k.ad, '') || ' ' || COALESCE(k.soyad, '') AS ad_soyad
...
exports.ataJuri = async (req, res) => {
  ...
  try {
    const check = await pool.query(
      'SELECT * FROM juri_atamalar WHERE ilan_id = $1 AND juri_id = $2',
      [ilanId, juriId]
    );
    ...
    await pool.query(
      'INSERT INTO juri_atamalar (ilan_id, juri_id) VALUES ($1, $2)',
      [ilanId, juriId]
    );
...
exports.silJuriAtama = async (req, res) => {
  ...
  try {
    await pool.query(
      'DELETE FROM juri_atamalar WHERE ilan_id = $1 AND juri_id = $2',
      [ilanId, juriId]
    );
